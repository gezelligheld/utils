/**
 * @file components/common/CropUpload/hooks/dragUpload 拖拽上传
 * @author maojiaqing
 */
 ...
 export default (props: DragProps) => {
     ...
     const handleDrop = useCallback((e: any) => {
         ...
         if (accept) {
             const types = _.map(accept.split(','), (type: string) => _.replace(type, /^\./, ''));
             let flag = true;
     ...
     const handleFileDrag = useCallback((e: any) => {
         e.preventDefault();
         e.stopPropagation();
